...
access(all) fun testSetupMultiContractMultiAccountUpdater() {
    ...
    // TODO: Negative case where no contract address contained for a given contract
    let deploymentConfig: [[{Address: {String: String}}]] = [stage0, stage1, stage2]
    let blockHeightBoundary: UInt64 = getCurrentBlock().height + blockHeightBoundaryDelay
    ...
    let setupUpdaterTxResult = executeTransaction(
        ...
        [blockHeightBoundary, contractAddresses, deploymentConfig],
        abcUpdater
    )
